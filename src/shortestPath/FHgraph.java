...
public class FHgraph<E>
{
    ...
    * @param x Name of the origin city
    * @param citiesToAvoid Arraylist of cities that we want to avoid
    * @return Output of the dijkstra algorithm
    **/
   public boolean dijkstra(E x, ArrayList<E> citiesToAvoid)
   {
      ...
      ArrayList<FHvertex> avoid = new ArrayList<FHvertex>();

      for (E city: citiesToAvoid) {
         if (getVertexWithThisData(city) != null) {
            avoid.add(getVertexWithThisData(city));
         }
      }
      ...
      while( !partiallyProcessedVerts.isEmpty() )
      {
         ...
         {
            edge = edgeIter.next();
            //add a condition that determines if the next edge contains a city to be avoided
            if (!avoid.contains(edge.first)) {
               w = edge.first;
               costVW = edge.second;
    ...
    * @param x1 Name of the origin city
    * @param x2 Name of the destination city
    * @param citiesToAvoid Arraylist of cities that we want to avoid
    * @return Output of the dijkstra algorithm
    **/
   ...
   /**
    * Calculates routes between origin and all other cities
    * @param x Name of the origin city
    * @param citiesToAvoid Arraylist of cities that we want to avoid
    * @return All shortest routes by using the dijkstra algorithm
    **/
   public boolean showDistancesTo(E x, ArrayList<E> citiesToAvoid)
   {
      ...
      if (!dijkstra(x, citiesToAvoid)) {
         System.out.println("No path exists for this starting city.");
         return false;
      }
